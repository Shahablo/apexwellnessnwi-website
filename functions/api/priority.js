...
function success(origin) {
  return json(origin, 200, {
    ok: true,
    message: "Thank you. Your priority-list request was received.",
  });
}
...
function validatePayload(payload, expectedConsentVersion) {
  ...
  const careInterest = typeof payload.care_interest === "string" && CARE_INTERESTS.has(payload.care_interest)
    ? payload.care_interest
    : null;
  ...
  if (!careInterest) fields.push("care_interest");
  if (payload.consent !== true) fields.push("consent");
  if (!consentVersion || (expectedConsentVersion && consentVersion !== expectedConsentVersion)) {
    fields.push("consent_version");
  }
...
function isLikelyBot(value, now) {
  if (value.website) return true;
  const elapsed = now - value.formStartedAt;
  return elapsed < MIN_FORM_TIME_MS || elapsed > MAX_FORM_AGE_MS;
}
...
async function handlePost(context, origin) {
  ...
  if (!env?.DB || typeof env.DB.prepare !== "function" || typeof env.DB.batch !== "function"
    ...
    return failure(origin, 503, "The priority list is temporarily unavailable. Please try again later.");
  }
  ...
  const validation = validatePayload(parsed.value, env.CONSENT_VERSION);
  if (validation.fields.length) {
    return failure(origin, 422, "Please check the highlighted fields and try again.", validation.fields);
  }

  const now = Date.now();
  if (isLikelyBot(validation.value, now)) return success(origin);
  ...
  const ipHash = await sha256(`${env.IP_HASH_SALT}:ip:${remoteIp}`);
  const emailHash = await sha256(`${env.IP_HASH_SALT}:email:${validation.value.email}`);
  ...
  await env.DB.prepare(INSERT_SUBMISSION_SQL).bind(
    crypto.randomUUID(),
    validation.value.fullName,
    validation.value.email,
    validation.value.careInterest,
  ...
  // Duplicate email addresses receive the same response to avoid disclosing list membership.
...
export async function onRequest(context) {
  ...
  try {
    ...
    console.error("Priority endpoint failure", error instanceof Error ? error.message : "unknown error");
    return failure(origin, 500, "The priority list is temporarily unavailable. Please try again later.");
  }
}
